...
int main()
{
    ...
    Pendulum p2(p1.GetBobCenter(), ConvertMeterToPixel(0.1));
    
    // pivotCoordinate.y += 20.0f;
    // pivotCoordinate.x += 150.0f;
    
    // p1.SetPivotCoordinate(pivotCoordinate); // not even needed. just used it for testing.
    
    p1.SetDisplayInfo((Vector2){15.0f, HEIGHT*0.2}, 20.0f, GREEN);
    p2.SetDisplayInfo((Vector2){15.0f, HEIGHT*0.8}, 20.0f, GREEN);
	...
	while(!WindowShouldClose())
	{
    	...
    	p2.SetPivotCoordinate(p1.GetBobCenter());
    	
    	p1.Update(input.GetFrameTime());
    	p2.Update(input.GetFrameTime());
